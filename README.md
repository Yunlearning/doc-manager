...
### 文件版本管理（Version Table Strategy）
- 📋 **版本歷史** — 每次上傳自動建立版本記錄，保留完整歷史
- ⏪ **版本還原** — 一鍵還原至任意歷史版本（建立新版本，不破壞歷史）
- 📝 **變更日誌** — 每次上傳可填寫 changelog 說明變更內容
- 👤 **上傳追蹤** — 記錄每個版本的上傳者資訊
...
| 方法 | 端點 | 說明 | 權限 |
|------|------|------|------|
| POST | `/documents/upload` | 上傳文件（支援新版本上傳） | UPLOAD |
| GET | `/documents?tierId=` | 取得階層下的文件列表 | 需登入 |
| GET | `/documents/jobs/:jobId` | 查詢上傳進度 | 需登入 |
| GET | `/documents/:id/versions` | 取得文件版本歷史 | 需登入 |
| POST | `/documents/:id/revert` | 還原至指定版本 | UPLOAD |
| GET | `/documents/:id/download` | 下載文件（串流） | DOWNLOAD |
| DELETE | `/documents/:id` | 刪除文件（含所有版本） | DELETE_DOCUMENT |
...
本系統採用 **Version Table Strategy**，將文件與版本分離儲存：
...
│─────────────────────│       │──────────────────────────│
│ id                  │──┐    │ id                       │
│ tierId              │  │    │ documentId (FK)          │
│ title               │  └───▶│ versionNumber            │
│ currentVersion (int)│       │ title / fileName / filePath │
│ uploadedById        │       │ mimeType / fileSize      │
...
**設計原則：**
- `Document` 為輕量級指標，僅記錄目前版本號
- 所有檔案資訊（路徑、大小、類型）存於 `DocumentVersion`
- 還原操作建立新版本（不破壞歷史紀錄）
- 上傳新版本時自動遞增 `currentVersion` 並建立新 `DocumentVersion`
...
├── backend/
│   ├── prisma/
│   │   ├── schema.prisma      # 資料庫 Schema（User, Document, DocumentVersion 等）
│   │   └── seed.ts            # 測試資料種子
│   ├── src/
│   │   ├── index.ts
│   │   ├── config/            # DB & Redis 設定
...
│   │   ├── queues/            # BullMQ 佇列
│   │   ├── jobs/              # 非同步工作（uploadWorker — 支援首次上傳與新版本上傳）
...
└── frontend/
    └── src/
        ...
        ├── types/             # TypeScript 型別（User, DocumentItem, DocumentVersion）
        └── validators/        # Zod 驗證
```
